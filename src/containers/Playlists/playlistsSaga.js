...
function* getUserTracks() {
  try {
    ...
    if (tracks) yield put(actions.getUserTracksSuccess({ playlist: { tracks } }));
  } catch (err) {
...
function* checkUserFollowThePlaylist({ payload: { playlistId, userId } }) {
  try {
    const following = yield services.checkUserFollowPlaylist(playlistId, userId);
    if (following) yield put(actions.checkUserFollowPlaylistSuccess({ following: following[0] }));
  } catch (err) {
...
function* checkUserFollowThePlaylistSaga() {
  yield takeLatest(constants.CHECK_USER_FOLLOW_PLAYLIST_START, checkUserFollowThePlaylist);
}

// follow unfollow
function* followUnfollowPlaylist({ payload: { playlistId, action = 'follow', userId } }) {
  try {
